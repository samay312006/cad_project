...
| 7 | New sketch `embedded/web_drive/` added — a browser-based drive-control + telemetry bring-up tool (Wi-Fi AP, WebSocket, HUD frontend), served from LittleFS. Written by the cloud/GUI session this time with explicit human authorization, as a normal exception (see design doc `docs/superpowers/specs/2026-09-03-web-drive-gui-design.md` §0). Reuses `drive_test.ino`/`drive_calib.ino`'s pin map and duty-clamp logic **unmodified from those files**, but duplicates it (not a shared header) — the two originals are untouched. Duty-clamp constants (`v0Drop`/`rTot`/`vBusManual`) are `drive_calib.ino`'s *defaults*, not a real calibration (`embedded/control/calibration.h` still doesn't exist). **Not yet bench-tested** — Task 7 of the implementation plan (physical hardware bring-up, wheels off the ground) is the next step, on real hardware, by whoever's at the bench next | Embedded engineer — new files in your normal directory you didn't write; flagging so it's not a surprise. Full design: `docs/superpowers/specs/2026-09-03-web-drive-gui-design.md`, plan: `docs/superpowers/plans/2026-09-03-web-drive-gui.md` | Whoever does Task 7 hardware bring-up next | 2026-09-03 |
