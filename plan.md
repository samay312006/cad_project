...
- **Range sensor + sweep servo**: **no standard Gazebo plugin exists for this either** (like the current/voltage node below, it's domain-specific to this project's cost-driven sensor choice). Build a synthetic node that models a single-point range reading (Gazebo's ray/contact sensor API can supply the raw distance) sampled as a simulated servo angle sweeps back and forth, emitting one `scan_sample`-shaped reading at a time (matching `spec.md` §6.1) at a rate that reflects a `[TBD]` sensor sample time × `[TBD]` servo slew speed — model this conservatively (slow, sparse) rather than optimistically as a drop-in for a real spinning 2D lidar, since the acceptance criteria in `spec.md` §8 (SLAM rate, Follow-me lock tolerance) are explicitly flagged as needing revalidation against this sensor's real achievable rate. Accumulate samples into a `sensor_msgs/LaserScan`-shaped message on the simulated-phone node before publishing to the topic that maps to `spec.md`'s `robot/{id}/scan` payload, matching the real ESP32→phone→cloud data path.
...
The important design decision here: **write the control-loop logic once, behind a hardware abstraction layer (HAL), and run it two ways** — natively as a Linux process/ROS2 node now, and later compiled for ESP-IDF/Arduino on real hardware. The mode state machine, incline-compensation controller, and fault/SAFE_HOLD logic from `spec.md` §5–§7 live in this portable core; only the HAL (I2C/UART/PWM drivers, and now a Wi-Fi UDP socket driver, vs. simulated topic I/O) differs between the two builds. This is the same "drop-in replacement" principle the project applies to EC2, pushed down one more level — without it, the local sim would validate a *different* control loop than what eventually ships to hardware, defeating the point of simulating first.

The simulated-phone node implements the same JSON-over-UDP contract from `spec.md` §6.1, but instead of a real Wi-Fi socket, use a plain ROS2 topic carrying the same JSON payloads as `std_msgs/String`, which is simpler to wire up and sufficient for validating message-contract correctness and control logic (it does not validate real Wi-Fi throughput/latency/range behavior, which is acceptable to defer to 2c hardware bring-up — see the Wi-Fi AP bring-up step added there).
...
3. **Follow-me** — synthetic moving target in Gazebo, confirm range-sensor-sweep-based lock acquisition/tracking/standoff distance per `spec.md` FR-4, and confirm safe fallback to MANUAL on lock loss. Given the sweep sensor's rate is far below a real 2D lidar's (2a.2), this exercise is also where the `[ASSUMPTION] 0.5 m` lock-tolerance figure in `spec.md` §8 should first get real (simulated) data against it — expect to revise that number, not just confirm it.
...
- **Range-sensor occlusion by the robot's own chassis or a carried payload**: define, in sim, a "self-occlusion" mask for known blind angles in the servo's sweep (e.g. directly behind if the electromagnet arm blocks part of the sweep) so SLAM/obstacle-detection code doesn't misinterpret a self-occlusion as a real nearby obstacle. Validate this mask against the actual sensor/servo mounting position once chosen `[TBD]`.
- **ESP32↔phone comms loss and fail-safe**: validated directly in 2a.5 exercise 6. The important invariant: the ESP32-sim (and later, real ESP32) must detect *absence* of a heartbeat itself — it cannot rely on the phone sending an explicit "I'm about to disconnect" message, since real failures (crash, app killed, phone reboot, phone walking out of Wi-Fi AP range, RF interference) don't announce themselves. Sim this as a plain heartbeat-stream cutoff — no need to model RF propagation in Gazebo; the real-world range/interference cases are for 2c hardware bring-up (2c.5) to validate, not this sim.
...
1. Bring up the ESP32-core logic (from 2a.3's HAL-separated build) on real hardware with **no sensors attached yet** — validate the mode state machine and SAFE_HOLD logic run and the heartbeat-timeout fail-safe fires correctly, using only a serial debug console for stimulus (this is a dev/debug UART for firmware bring-up, separate from the wireless phone bridge in step 5).
...
3. Add IMU, then GPS, then the current/voltage sensing chain, then the range sensor + sweep servo, one at a time, validating each against the sim's expected message shape from `spec.md` §6.1 before adding the next. For the range sensor + servo specifically, bench-measure the actual achievable scan rate (sensor sample time × servo slew speed) before trusting any downstream SLAM/Follow-me acceptance criteria that depend on it (`spec.md` §8).
...
5. Only after 1–4 pass individually: bring up the ESP32's Wi-Fi AP and confirm the phone can associate, authenticate (WPA2/WPA3, `spec.md` §6.1), and exchange the JSON-over-UDP contract at expected latency (NFR-3) — then re-run the full mode-transition matrix from `spec.md` §5 against **real** hardware, the same test suite already proven in sim (2a.5).
...
- **ESP32↔phone comms loss, real hardware**: re-validate the exact scenario proven in sim (2a.6) on real hardware, now with a broader set of real causes than a wired link had — power off the phone's Wi-Fi during Autonomous/Follow-me, walk the phone out of AP range, and (if feasible) introduce 2.4/5 GHz interference (e.g. another AP or a microwave-class source) — and confirm SAFE_HOLD entry within the NFR-2 budget in each case, not just in sim timing. Also confirm the AP requires WPA2/WPA3 auth in the field build, not a dev-convenience open AP left over from bring-up.
...
- **Range-sensor occlusion by the robot's own chassis/payload, real hardware**: re-derive the self-occlusion mask (2a.6) against the actual sensor/servo mounting position and actual electromagnet arm geometry once both are physically fixed — the sim mask was necessarily provisional. Also re-measure the real achievable sweep rate/resolution on hardware (2c.3 step 3) and feed that back into the `spec.md` §8 SLAM and Follow-me acceptance criteria, which were explicitly left open pending this data.
